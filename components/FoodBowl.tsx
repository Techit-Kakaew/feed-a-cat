...
export default function FoodBowl({ onAddFood, foodAmount }: FoodBowlProps) {
  ...
  return (
    <div className="relative flex flex-col items-center gap-1 md:gap-6">
      {/* Prominent Unit Display */}
      <div className="flex flex-col items-center bg-white/5 backdrop-blur-xl rounded-2xl px-4 py-2 md:px-8 md:py-4 border border-white/10 shadow-2xl">
        <div className="text-3xl md:text-6xl font-black text-transparent bg-clip-text bg-linear-to-b from-yellow-200 to-yellow-500 drop-shadow-sm font-mono tracking-tighter">
          <RollingNumber value={foodAmount} />
        </div>
        <div className="text-[7px] md:text-[10px] font-black text-yellow-400/60 tracking-[0.3em] uppercase mt-0.5 md:mt-1">
          Kitty Kibbles
        </div>
      </div>
      ...
      <div className="relative group">
        ...
        <motion.div
          className="absolute inset-0 rounded-full border-2 md:border-4 border-yellow-400/0 group-hover:border-yellow-400/20 transition-all duration-700"
          animate={{
            scale: [1, 1.2, 1],
            opacity: [0, 0.5, 0],
          }}
        ...
        <AnimatePresence>
          {particles.map((particle) => (
            ...
            >
              <span className="text-3xl md:text-5xl font-black text-yellow-400 drop-shadow-[0_4px_12px_rgba(0,0,0,0.5)] select-none">
                +1
              </span>
            </motion.div>
          ))}
        ...
        <motion.button
          ...
          className="relative cursor-pointer select-none touch-manipulation 
                     ...
                     rounded-full p-6 md:p-10 border-[3px] md:border-4 border-amber-900 
                     shadow-[0_15px_40px_rgba(0,0,0,0.4),inset_0_-6px_10px_rgba(0,0,0,0.15),inset_0_6px_10px_rgba(255,255,255,0.9)]
        ...
        >
          <div className="text-5xl md:text-8xl drop-shadow-xl filter saturate-125">
            🥣
          </div>
